...
library.Icons = {
    Aimbot = "http://www.roblox.com/asset/?id=7300477598",
    ESP = "http://www.roblox.com/asset/?id=7300535052",
    Folder = "http://www.roblox.com/asset/?id=7300480952",
    Misc = "http://www.roblox.com/asset/?id=7300486042",
                ...
                function sector.element(type, text, data, callback, c_flag)
                    ...
                    elseif type == "MultiDropdown" then
                        ...
                        for _, v in ipairs(options) do
                            ...
                            Button.MouseLeave:Connect(function()
                                if not table.find(value.MultiDropdown, v) then
                                    library:tween(ButtonText, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(150, 150, 150)})
                                    Decoration.Visible = false
                                end
                            end)
                            Button.MouseButton1Down:Connect(function()
                                if table.find(value.MultiDropdown, v) then
                                    table.remove(value.MultiDropdown, table.find(value.MultiDropdown, v))
                                    Decoration.Visible = false
                                    library:tween(ButtonText, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(150, 150, 150)})
                                else
                                    table.insert(value.MultiDropdown, v)
                                    Decoration.Visible = true
                                    library:tween(ButtonText, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(255, 255, 255)})
                                end

                                local display = {}
                                for i, txt in ipairs(value.MultiDropdown) do
                                    if i <= 3 then table.insert(display, txt) end
                                end
                                if #value.MultiDropdown > 3 then
                                    table.insert(display, "...")
                                end
                                DropdownButtonText.Text = #display > 0 and table.concat(display, ", ") or "..."
                        ...
                        function element:set_value(new_value, cb)
                            ...
                            for _, child in pairs(DropdownScroll:GetChildren()) do
                                if child:IsA("TextButton") then
                                    ...
                                    if btnText and table.find(value.MultiDropdown, btnText.Text) then
                                        child.Decoration.Visible = true
                                        ...
                                        child.Decoration.Visible = false
                                        if btnText then
                        ...
                        element:set_value(value, true)
                    elseif type == "SmoothGraph" then
                        Border.Size = Border.Size + UDim2.new(0, 0, 0, 60)

                        value = {SmoothGraph = default and default.default or 0.15}

                        local min, max = 0.01, 0.5
                        ...
                        local SliderValue = library:create("TextLabel", {
                            ...
                            Size = UDim2.new(0, 200, 0, 9),
                            Font = Enum.Font.Ubuntu,
                            Text = string.format("%.2f", value.SmoothGraph),
                            TextColor3 = Color3.fromRGB(150, 150, 150),
                            TextSize = 14,
                            TextXAlignment = Enum.TextXAlignment.Right,
                        }, Slider)
                        ...
                        SliderButton.MouseButton1Down:Connect(function()
                            ...
                            value.SmoothGraph = val
                            SliderValue.Text = string.format("%.2f", val)
                            do_callback()
                            is_sliding = true
                        ...
                        function element:set_value(new_value, cb)
                            value = new_value and new_value or value
                            menu.values[tab.tab_num][section_name][sector_name][flag] = value
                            local new_size = (value.SmoothGraph - min) / (max - min)
                            SliderFrame.Size = UDim2.new(new_size, 0, 1, 0)
                            SliderValue.Text = string.format("%.2f", value.SmoothGraph)
                            if cb == nil or not cb then
                                do_callback()
                            end
                        end
                        element:set_value(value, true)
                    elseif type == "BezierCurve" then
                        Border.Size = Border.Size + UDim2.new(0, 0, 0, 80)

                        value = {BezierCurve = default and default.default or {0, 0, 1, 1}}
                        ...
                        local CurveFrame = library:create("Frame", {
                            Name = "CurveFrame",
                            BackgroundColor3 = Color3.fromRGB(25, 25, 25),
                            BorderColor3 = Color3.fromRGB(0, 0, 0),
                            Position = UDim2.new(0, 9, 0, 20),
                        ...
                        local CurveCanvas = library:create("ImageLabel", {
                            Name = "CurveCanvas",
                            BackgroundTransparency = 1,
                            Size = UDim2.new(1, 0, 1, 0),
                            Image = "rbxassetid://4550859311",
                        ...
                        local CurveLine = library:create("Frame", {
                            Name = "CurveLine",
                            BackgroundColor3 = Color3.fromRGB(84, 101, 255),
                            BorderSizePixel = 0,
                        }, LineFrame)
                        ...
                        Handle2.MouseButton1Down:Connect(function()
                            dragging = 2
                        end)
                        ...
                        function element:set_value(new_value, cb)
                            value = new_value and new_value or value
                            menu.values[tab.tab_num][section_name][sector_name][flag] = value
                            Handle1.Position = UDim2.new(value.BezierCurve[1], 0, 1 - value.BezierCurve[2], 0)
                            Handle2.Position = UDim2.new(value.BezierCurve[3], 0, 1 - value.BezierCurve[4], 0)
                            updateLine()
                            if cb == nil or not cb then
                                do_callback()
                            end
                    ...
                    elseif type == "ColorPicker" then
                        ...
                        local ColorButton = library:create("TextButton", {
                            Name = "ColorButton",
                            BackgroundColor3 = value.ColorPicker,
                            BorderColor3 = Color3.fromRGB(0, 0, 0),
                            Position = UDim2.new(0, 230, 0, 5),
                        ...
                        local h, s, v = value.ColorPicker:ToHSV()
                        h = math.clamp(h, 0, 1)
                        s = math.clamp(s, 0, 1)
                        v = math.clamp(v, 0, 1)
                        ...
                        ColorButton.MouseButton1Down:Connect(function()
                            if not ColorPicker then
                                ColorPicker = library:create("Frame", {
                                    Name = "ColorPicker",
                                    BackgroundColor3 = Color3.fromRGB(10, 10, 10),
                                    BorderColor3 = Color3.fromRGB(0, 0, 0),
                                ...
                                HuePick = library:create("ImageButton", {
                                    Name = "HuePick",
                                    BackgroundColor3 = Color3.fromRGB(255, 255, 255),
                                    BorderColor3 = Color3.fromRGB(0, 0, 0),
                                    Size = UDim2.new(1, 0, 0, 1),
                        ...
                        uis.InputBegan:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
                                if ColorPicker and ColorPicker.Visible and not in_color and not in_color2 then
                        ...
                        function element:set_value(new_value, cb)
                            value = new_value and new_value or value
                            menu.values[tab.tab_num][section_name][sector_name][flag] = value
                            ColorButton.BackgroundColor3 = value.ColorPicker
                            if ColorPickFrame then ColorPickFrame.ImageColor3 = Color3.fromHSV(h, 1, 1) end
                            if cb == nil or not cb then
                                do_callback()
                            end
                        ...
                        function element:Set(color)
                            ...
                            s = math.clamp(s, 0, 1)
                            v = math.clamp(v, 0, 1)
                            if ColorPick then ColorPick.Position = UDim2.new(1 - s, 0, 1 - v, 0) end
                            if HuePick then HuePick.Position = UDim2.new(0, 0, 1 - h, -1) end
                            if ColorPickFrame then ColorPickFrame.ImageColor3 = Color3.fromHSV(h, 1, 1) end
                        end
                    elseif type == "Input" then
                        ...
                        local InputText = library:create("TextLabel", {
                            Name = "InputText",
                            BackgroundTransparency = 1,
                            Position = UDim2.new(0, 9, 0, 8),
                            Size = UDim2.new(0, 200, 0, 15),
                        ...
                        InputBox.Focused:Connect(function()
                            library:tween(InputBox, TweenInfo.new(0.2), {BorderColor3 = Color3.fromRGB(84, 101, 255)})
                        end)
                        InputBox.FocusLost:Connect(function()
                            library:tween(InputBox, TweenInfo.new(0.2), {BorderColor3 = Color3.fromRGB(0, 0, 0)})
                            value.Input = InputBox.Text
                            do_callback()
                        end)
