...
function esp:raycast(a, b, c)
    ...
    -- Iterative raycast to avoid recursion and potential stack overflows
    local MAX_STEPS = 25
    local step = 0
    while step < MAX_STEPS do
        ...
        if hit and hit.Transparency ~= nil and hit.Transparency >= 0.250 then
            TINSERT(c, hit)
            -- continue loop with updated blacklist
            step = step + 1
        else
            return ray
        end
