	...
	fpsLabel.Text = "FPS: " .. tostring(math.floor(realFPS))
        ...
        -- Elements  
            ...
            function library:label(options)
                ...
                local frame = library:create("TextLabel", {
                    FontFace = fonts["ProggyClean"];
            ...
            function library:searchbox(options)
                local cfg = {
                    ...
                    callback = options.callback or function() end,
                    items = options.items or {},
                    filter_callback = options.filter_callback or function() end,
                    option_instances = {},
                }
                ...
                local input_frame = library:create("TextButton", {
                    AnchorPoint = vec2(1, 0);
                    Text = "";
                    AutoButtonColor = false;
                    Parent = frame;
                    ...
                    BorderSizePixel = 0;
                    BackgroundColor3 = themes.preset.inline
                }); library:apply_theme(input_frame, "inline", "BackgroundColor3")

                local search_icon = library:create("TextLabel", {
                    FontFace = fonts["ProggyClean"];
                    TextColor3 = rgb(170, 170, 170);
                    BorderColor3 = rgb(0, 0, 0);
                    Text = "🔍";
                ...
                input:GetPropertyChangedSignal("Text"):Connect(function()
                    ...
                    if cfg.filter_callback then
                        cfg.filter_callback(input.Text)
                    end
                end)
            ...
            function library:number_input(options)
                local cfg = {
                ...
                local input_frame = library:create("TextButton", {
                    ...
                    BorderSizePixel = 0;
                    BackgroundColor3 = themes.preset[tostring(self.count)]
                }); library:apply_theme(input_frame, tostring(self.count), "BackgroundColor3")
                ...
                local function clamp_value(val)
                    return math.max(cfg.min, math.min(cfg.max, val))
                end

                local function round_value(val)
                    return math.round(val / cfg.step) * cfg.step
                end
                ...
                function cfg.set(value)
                    ...
                    cfg.value = clamp_value(round_value(num))
                    input.Text = tostring(cfg.value)
                    flags[cfg.flag] = cfg.value
                    cfg.callback(cfg.value)
                end

                function cfg.increment()
                    cfg.set(cfg.value + cfg.step)
                end

                function cfg.decrement()
                    cfg.set(cfg.value - cfg.step)
                end
                ...
                input_frame.MouseButton1Click:Connect(function()
                    input:CaptureFocus()
                end)
            ...
            function library:toggle_with_description(options)
                local cfg = {
                    enabled = options.enabled or nil,
                    name = options.name or "Toggle",
                ...
                if cfg.description and cfg.description ~= "" then
                    local desc_label = library:create("TextLabel", {
                        FontFace = fonts["ProggyClean"];
            ...
            function library:searchable_dropdown(options)
                local cfg = {
                    name = options.name or nil,
                    flag = options.flag or options.name or "Flag",
                    items = options.items or {""},
                ...
                local dropdown_holder = library:create("TextButton", {
                    ...
                    Text = "";
                    Parent = frame;
                    Position = dim2(1, 0, 0, 14);
                    BorderColor3 = rgb(0, 0, 0);
                    Size = dim2(0.5, 0, 0, 20);
                    BorderSizePixel = 0;
                    BackgroundColor3 = themes.preset[tostring(self.count)]
                }); library:apply_theme(dropdown_holder, tostring(self.count), "BackgroundColor3")
                ...
                local inline_dropdown = library:create("Frame", {
                    Parent = accent;
                    Size = dim2(1, -2, 1, -2);
                    Position = dim2(0, 1, 0, 1);
                    BorderColor3 = rgb(0, 0, 0);
                    ...
                    AutomaticSize = Enum.AutomaticSize.Y;
                    BackgroundColor3 = themes.preset.inline
                }); library:apply_theme(inline_dropdown, "inline", "BackgroundColor3")
                ...
                function cfg.set(value)
                    ...
                    for _, option in next, cfg.option_instances do
                        if option.Text and (option.Text == value or (isTable and find(value, string.gsub(option.Text, "^%s+", "")))) then
                            insert(selected, string.gsub(option.Text, "^%s+", ""))
                            cfg.multi_items = selected
                            option.TextColor3 = rgb(255, 255, 255)
                        else
                            option.TextColor3 = rgb(170, 170, 170)
                    ...
                    local display_text = if isTable then concat(selected, ", ") else selected[1]
                    search_input.Text = display_text or ""
                ...
                function cfg.refresh_options(list)
                    ...
                    for _, option in next, list do
                        ...
                        button.MouseButton1Down:Connect(function()
                            if cfg.multi then
                                local clean_text = string.gsub(button.Text, "^%s+", "")
                                local selected_index = find(cfg.multi_items, clean_text)

                                if selected_index then
                                    remove(cfg.multi_items, selected_index)
                                else
                                    insert(cfg.multi_items, clean_text)
                                end
                ...
                function cfg.filter_options(text)
                    ...
                    for _, option in next, cfg.option_instances do
                        if string.find(string.lower(option.Text), text) then
                            option.Visible = true
                        else
                ...
                search_input:GetPropertyChangedSignal("Text"):Connect(function()
                    cfg.filter_options(search_input.Text)
                end)
                ...
                uis.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        if not (library:mouse_in_frame(accent) or library:mouse_in_frame(frame)) then
                            cfg.open = false
                            cfg.set_visible(false)
                        end
                    end
            ...
            function library:multiselect_dropdown(options)
                local cfg = {
                    name = options.name or nil,
                    flag = options.flag or options.name or "Flag",
                    items = options.items or {""},
                ...
                function cfg.render_option(text, is_selected)
                    local option = library:create("Frame", {
                        Parent = scrollingframe;
                        Size = dim2(1, 0, 0, 20);
                        BackgroundTransparency = 1;
                        BorderSizePixel = 0;
                    ...
                    local checkbox = library:create("Frame", {
                        ...
                        BorderColor3 = rgb(0, 0, 0);
                        BorderSizePixel = 0;
                        BackgroundColor3 = is_selected and themes.preset[tostring(self.count)] or themes.preset.inline
                    }); library:apply_theme(checkbox, tostring(self.count), "BackgroundColor3")
                    ...
                    local label = library:create("TextButton", {
                        FontFace = fonts["ProggyClean"];
                        AutoButtonColor = false;
                        TextColor3 = is_selected and rgb(255, 255, 255) or rgb(170, 170, 170);
                        BorderColor3 = rgb(0, 0, 0);
                        Text = text;
                        Parent = option;
                        Size = dim2(1, -20, 1, 0);
                    ...
                    return {frame = option, checkbox = checkbox, label = label, text = text}
                end
                ...
                function cfg.refresh_options(list)
                    ...
                    for _, option_text in next, list do
                        local is_selected = find(cfg.selected_items, option_text) ~= nil
                        local option = cfg.render_option(option_text, is_selected)

                        insert(cfg.option_instances, option)

                        option.label.MouseButton1Down:Connect(function()
                            local selected_index = find(cfg.selected_items, option_text)

                            if selected_index then
                                remove(cfg.selected_items, selected_index)
                            else
                                insert(cfg.selected_items, option_text)
                            end

                            cfg.refresh_options(list)
                            cfg.update_text()
                            cfg.callback(cfg.selected_items)
                            flags[cfg.flag] = cfg.selected_items
                        end)
                    end
                end

                function cfg.update_text()
                    if #cfg.selected_items == 0 then
                        text.Text = "None selected"
                    elseif #cfg.selected_items == 1 then
                        text.Text = cfg.selected_items[1]
                    else
                        text.Text = #cfg.selected_items .. " selected"
                    end
                end
                ...
                function cfg.set(items)
                    if type(items) ~= "table" then
                        items = {items}
                    end

                    cfg.selected_items = items
                    cfg.refresh_options(cfg.items)
                    cfg.update_text()
                    flags[cfg.flag] = cfg.selected_items
                    cfg.callback(cfg.selected_items)
