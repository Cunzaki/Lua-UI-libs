...
function Library:CreateTab(config)
    ...
    function tab:CreateSection(config)
        return Library:CreateSection(config, self)
    end
...
function Library:CreateSection(config, tab)
    ...
    if not sideSections then
        error("Invalid section position: " .. tostring(position))
        return
    end
    if #sideSections >= 1 then
    ...
    if CurrentTab and tab == CurrentTab and table.find(tab.sections[position], section) == nil then
    ...
    table.insert(tab.sections[position], section)
